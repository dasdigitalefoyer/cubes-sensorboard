...
#define SH2_NO_CHANGE                 (100)  /**< Configuration not written, no chang after read */
...
public:
  ...
  // added by PP BEGIN - configuring Sensors if changed (does a readout)
  int configureStabilityDetector(float accelerationThreshold = 0.784f /* m/s^2 */,float duration  = 0.5 /*s*/);
  int configureMotionEngine(float deltaOrientation = 0.2f, float stableThreshold = 1, uint32_t deltaAcceleration = 0, uint32_t stableDuration = 3);
  // added by PP END
