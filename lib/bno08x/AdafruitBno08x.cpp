 ...
 *  modified by Patrick Pogscheba (HSD)
 *
 ...
 *
 *     v1.0 - First release
 *     v1.1 - modfied release (added configuration (ME,Stability)
 */
...
// added by PP BEGIN
static int16_t floatToQ(float qFloat, uint8_t qPoint)
{
	int16_t qVal = static_cast<int16_t>(qFloat * pow(2.0f, qPoint));
	return qVal;
}

static int32_t floatToQ_dword(float qFloat, uint16_t qPoint)
{
  ...
  // a [m/s^2], d [s]
int Adafruit_BNO08x::configureStabilityDetector(float accelerationThreshold , float duration) 
{



  //ID:ME_POWER_MGMT
  uint32_t pDataCurrent[2];
  uint32_t pData[2];
  uint16_t words;
  int ret = sh2_getFrs(STABILITY_DETECTOR_CONFIG,pDataCurrent , &words);
  if(ret != SH2_OK)
  {
    pDataCurrent[0] = static_cast<uint32_t>(floatToQ_dword(0.784f,24));
    pDataCurrent[1] = static_cast<uint32_t>(floatToQ_dword(500000.0f,0));
  }
  
  pData[0] = static_cast<uint32_t>(floatToQ_dword(accelerationThreshold,24));
  pData[1] = static_cast<uint32_t>(floatToQ_dword(duration * 1000000.0f,0));
  
  
  ret = SH2_NO_CHANGE;
  if(pData[0] != pDataCurrent[0] || pData[1] != pDataCurrent[1] )

    ret = sh2_setFrs(STABILITY_DETECTOR_CONFIG,pData , 2);
  return ret;
 
}


int Adafruit_BNO08x::configureMotionEngine(float deltaOrientation, float stableThreshold, uint32_t deltaAcceleration, uint32_t stableDuration)
{

  //ID:ME_POWER_MGMT
  uint32_t pDataCurrent[3];
  uint32_t pData[3];
  uint16_t words;
  int ret = sh2_getFrs(ME_POWER_MGMT,pDataCurrent , &words);
  if(ret != SH2_OK)
  {
    pDataCurrent[0] = static_cast<uint32_t>(floatToQ_dword(0.2f,28));
    pDataCurrent[1] = static_cast<uint32_t>(floatToQ_dword(1,25));
    pDataCurrent[2] = static_cast<uint32_t>(3 | (0 << 16));
  }
  
 
  pData[0] = static_cast<uint32_t>(floatToQ_dword(deltaOrientation,28));
  pData[1] = static_cast<uint32_t>(floatToQ_dword(stableThreshold,25));
  pData[2] = static_cast<uint32_t>(stableDuration | (deltaAcceleration << 16));
  
  ret = SH2_NO_CHANGE;
  if(pData[0] != pDataCurrent[0] || pData[1] != pDataCurrent[1]  || pData[2] != pDataCurrent[2] )

    ret = sh2_setFrs(ME_POWER_MGMT,pData , 3);
  return ret;
